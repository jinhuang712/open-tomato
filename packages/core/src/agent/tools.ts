    ...
    tools.push(
      defineTool({
        ...
        execute: async (toolCallId, params, signal) => {
          ...
          const preview = await store.previewWrite(kind, params.id, params.content);
  ...
  if (perms.canAsk) {
    tools.push(
      defineTool({
        name: "ask_user",
        label: "问作者",
        description: "向作者提一个问题，等作者在界面上回答。给 options 时作者可以点选；allowFreeText 为 true 时也可以自由输入。",
        parameters: Type.Object({
          question: Type.String(),
          options: Type.Optional(Type.Array(Type.String(), { description: "可选项，2–6 个" })),
