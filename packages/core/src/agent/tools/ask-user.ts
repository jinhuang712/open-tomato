...
export function makeAskUserTool(ctx: ToolContext): ToolDefinition {
  return defineTool({
    ...
    label: "问作者",
    description:
      "向作者提问并等待回答，用于澄清、讨论和创作选择。say 支持完整的 Markdown 解释，与普通对话采用相同的沟通原则，无需另调 say。question 放问题本身。按问题形态选 kind：\n" +
      "| kind | 作者的动作 | options |\n|---|---|---|\n" +
    ...
    parameters: Type.Object({
      say: Type.Optional(Type.String({ description: "对作者的解释或讨论，支持完整 Markdown；无需补充时可省略。" })),
      explainedReports: explainedReportsSchema,
      question: Type.String({ description: "清楚、容易回答的问题；解释放 say" }),
      kind: Type.Optional(
        Type.Union(
    ...
    execute: async (_id, params, signal) => {
      // 未解释报告不阻断澄清或探索；解释声明仍需合法，不能顺带清空报告。
      validateExplainedReports(ctx, params.say, params.explainedReports);
