...
export function makeContinueAgentTool(ctx: ToolContext): ToolDefinition {
  ...
  return defineTool({
    ...
    label: "续派子 agent",
    description:
      "接着和一个已经跑完一轮的子 agent 说话，它带着之前的上下文继续干。用于：它给了候选、作者拍板后让它在选中的候选上孵化落盘；或让它按作者意见修改。派出后立刻返回，报告做完后作为新消息送到你这里。agentId 从派单名册或它报告的标题里取。不要用它重派一个全新的任务。",
    parameters: Type.Object({
      agentId: Type.String({ description: "派单名册或子 agent 报告标题里的 id" }),
    ...
    execute: async (_id, params, _signal, onUpdate) => {
      const result = await continueAgent(params.agentId, params.message, params.mode, (progress, details) => onUpdate?.({ ...text(progress), details }));
