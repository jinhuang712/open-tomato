...
/**
 * 主编这一轮该不该补一句：没调 ask_user、不是出错或暂停、没有子 agent 在跑、收件箱里也没有作者的话等着（有就送作者的话，不用补）。
 * 每次作者发言只补一次，补完再停就真停，交给作者。
 */
export function shouldNudge(live: Pick<LiveAgent, "info" | "asked" | "nudged" | "hold" | "inbox">, childrenRunning = false): boolean {
