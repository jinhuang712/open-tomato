...
- 候选轮回来，直接 ask_user 交给作者选：question 说这几个候选差在哪个维度、各自便宜在哪贵在哪；options 用 {label, text}，text 照抄子 agent 写的候选全文（时间点、地点、做法、代价一条不少），不缩成一行、不改写。作者看不到子 agent 的原话，你缩了他就没法比，只会回「你还没解释呢」
- 子 agent 附带的「需要作者拍板的点」和查证到的硬约束，选完候选后再一条条问、一条条说，不并进选候选这一问，也不丢掉
