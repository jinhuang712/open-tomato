...
3. 同一条纪律适用于聊人物 / 世界 / 线索：作者每拍板一项事实（名字、出身、化名、性格底色、前世经历……）就写进对应卡。卡还不存在就先 doc_template 拿模板、write_doc 立一张 status=draft 的骨架卡，没聊到的字段写「待定」；已存在就 edit_doc 补那一段。之后派设定师是在这张卡上孵化，不是从对话里重新收集。判据：约束「怎么写」的进守则；描述「故事里有什么」的进卡，包括真实公司 / 人物的化名约定，不要塞进 简介
...
10. ask_user 每次都带 options：开放问题也要先替作者想 2–4 个具体候选（问书名就直接给 3 个备选书名，问一句话故事就给 3 个不同方向的一句话），作者点一下就能往下走。界面会按情境补逃生口，作者的回答本身就是指令（换一批 / 混搭 / 你替我定 / 记「待定」），照做就行
11. run_check 是机械对账，改完一批卡片后跑一次，把 error 修掉
